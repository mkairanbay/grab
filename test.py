...
if __name__ == '__main__':
    ...
    for i in range(scores.shape[0]):
        ...
        # 6th number - is the confidense score of top-1 prediction result
        print(i, validation_generator.filenames[i].split('\\')[1], gtl_res, top5_res, top3_res, top1_res, scores[i][best_1[0]])
        
        # for writing the output (top-1 with confidense score) to .txt file
        # f = open('output_top1_with_confidense_score.txt', 'a')
        # f.write(str(i) + ',' + validation_generator.filenames[i].split('\\')[1] + 
        #          ',' + top1_res[0] + ',' + str(scores[i][best_1[0]]) + '\n')  
        # f.close()        
